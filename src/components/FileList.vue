<template>
  <div v-if="files.length > 0" class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-gray-800">文件列表</h3>
            <p class="text-sm text-gray-500">{{ files.length }} 个文件</p>
          </div>
        </div>
        <button
          @click="handleClearFiles"
          class="px-3 py-1.5 text-sm text-gray-500 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors flex items-center gap-1"
        >
    ...
    <ul class="divide-y divide-gray-100">
      ...
      >
        <div class="flex items-center gap-4">
          <!-- 缩略图 -->
          <div class="w-16 h-16 flex-shrink-0 bg-gray-100 rounded-xl overflow-hidden shadow-sm">
            <img
              :src="file.previewUrl"
              :alt="file.name"
          ...
          <div class="flex-1 min-w-0">
            ...
            <p v-if="file.error" class="text-sm text-red-500 mt-1 flex items-center gap-1">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex items-center gap-2">
            <button
              ...
              class="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-green-500 to-emerald-500 rounded-lg hover:shadow-lg hover:shadow-green-500/30 transition-all duration-200 flex items-center gap-1.5"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const getSizeChange = (original: number, converted?: number) => {
  ...
  if (change > 0) {
    return `<span class="text-red-500">+${change.toFixed(1)}%</span>`;
  } else if (change < 0) {
    return `<span class="text-green-500">${change.toFixed(1)}%</span>`;
  }
