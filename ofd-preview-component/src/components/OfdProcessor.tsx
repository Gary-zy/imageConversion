...
export interface OfdProcessorProps {
  className?: string;
  style?: React.CSSProperties;
  onFileLoaded?: (file: File, converter: OfdConverter) => void;
  onConvertComplete?: (blob: Blob | Blob[], format: OfdTargetFormat) => void;
  onError?: (error: string) => void;
}

export const OfdProcessor: React.FC<OfdProcessorProps> = ({
  className = '',
  style = {},
  onFileLoaded,
  onConvertComplete,
  ...
  const handleFileUpload = async (uploadedFile: File) => {
    ...
    try {
      ...
      setConverter(conv);
      setCurrentPage(0);
      onFileLoaded?.(uploadedFile, conv);
    } catch (error) {
      const errorMsg = `文件加载失败: ${error instanceof Error ? error.message : '未知错误'}`;
      alert(errorMsg);
      onError?.(errorMsg);
    } finally {
      setConverting(false);
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      onConvertComplete?.(result, targetFormat);
    } catch (error) {
  ...
  return (
    <div className={`ofd-processor max-w-7xl mx-auto p-6 ${className}`} style={style}>
      {/* 文件上传区域 */}
      {!converter && (
        <div
          onDrop={handleDrop}
          onDragOver={handleDragOver}
          className="ofd-upload-area border-2 border-dashed border-gray-300 rounded-lg p-12 text-center cursor-pointer hover:border-blue-500 transition-colors"
      ...
      {converting && !converter && (
        <div className="ofd-loading-progress bg-white rounded-lg p-6 shadow-sm">
      ...
      {/* 主内容区域 */}
      {converter && (
        <div className="ofd-main-content flex flex-col gap-6">
          {/* 预览区域 - 上方 */}
          <div className="ofd-preview-section w-full">
          ...
          {/* 设置区域 - 下方 */}
          <div className="ofd-settings-section w-full">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* 文件信息 */}
              <div className="ofd-file-info bg-white rounded-lg p-4 shadow-sm h-fit">
                ...
                <button
                  onClick={handleReset}
                  className="ofd-reset-btn mt-3 px-4 py-2 text-sm bg-gray-200 hover:bg-gray-300 rounded transition-colors"
                >
                  重新上传
                </button>
              </div>
              ...
              {/* 转换设置 */}
              <div className="ofd-conversion-settings bg-white rounded-lg p-4 shadow-sm">
            ...
            {/* 转换按钮 */}
            <div className="ofd-convert-btn-wrapper mt-6">
              <button
            ...
            {/* 转换进度 */}
            {converting && converter && (
              <div className="ofd-convert-progress mt-4 bg-white rounded-lg p-4 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium">{progress.status}</span>
                  <span className="text-sm text-gray-600">
                    {progress.current} / {progress.total}
